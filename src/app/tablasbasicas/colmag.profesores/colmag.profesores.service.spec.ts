...
describe('ColmagProfesoresService', () => {
    ...
    it('#save-Add should return a Add row', (done: DoneFn) => {
        ...
        let row = new ColmagProfesoresModel(rowBase);
        row._estado = 'N';
        delete row.ColmagProfesorId;

        //Add - ColmagProfesores
    ...
    it('#save-update should return 404 Not found', (done: DoneFn) => {
        ...
        let row = new ColmagProfesoresModel({});
        row.ColmagProfesorId = -1;
        ...
        service.save(row, row).subscribe(
            ...
            (error) => {
                fail(`expected an error for update, not ColmagProfesoresModel ${JSON.stringify(error)}`);
                done();
            }
        );
    });
