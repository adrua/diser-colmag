...
@Component({
  selector: 'colmag-inscripciones-table',
  templateUrl: './colmag.inscripciones.table.html',
  // styleUrls: ['./colmag.inscripciones.table.css'],
  providers: [ColmagInscripcionesService]
})
export class ColmagInscripcionesTable implements AfterViewInit {
    rows: ColmagInscripcionesModel[] = [];
    originals: ColmagInscripcionesModel[] = [];
    selectedRow: ColmagInscripcionesModel;
    selectedIndex: number = 0;
    originalRow: ColmagInscripcionesModel;

    public displayedColumns: string[] = ['colmagInscripcionFecha', 'colmagInscripcionNombre', 'colmagInscripcionApellido', 'colmagInscripcionCedula', 'colmagInscripcionEdad', 'colmagCasas/colmagCasaNombre'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.colmagInscripcionesService.getList(_filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new ColmagInscripcionesModel();
      this.originalRow = new ColmagInscripcionesModel();
    ...
    onSelect(e: Event, row: ColmagInscripcionesModel, index: number) {
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = ColmagInscripcionesModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: ColmagInscripcionesModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = ColmagInscripcionesModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.colmagInscripcionesService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(ColmagInscripcionesModel.cloneExcel(data.value), 'Inscripciones');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.colmagInscripcionesService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new ColmagInscripcionesModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new ColmagInscripcionesModel().fromClipboard(line);
            });

            this.colmagInscripcionesService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for ColmagInscripciones');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(ColmagInscripcionesDialog, {
          data: {
